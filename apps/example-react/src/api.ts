...
export async function fetchArticles(user: DemoUser): Promise<Article[]> {
  const response = await fetch(`${API_URL}/articles`, { headers: authHeaders(user) });
...
export async function fetchAbility(
  user: DemoUser,
): Promise<{ ability: AppAbility; rawRules: PackRule<RawRuleOf<AppAbility>>[] }> {
  const response = await fetch(`${API_URL}/me/abilities`, { headers: authHeaders(user) });
  const data = (await response.json()) as { rules: PackRule<RawRuleOf<AppAbility>>[] };
